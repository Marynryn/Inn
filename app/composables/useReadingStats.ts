...
export const encouragement = (stats: Stats) => {
  ...
  if (p <= 0) return 'Всё впереди. Дверь трактира открыта.'
  if (p < 10) return 'Первые шаги. В трактире уже знают ваше имя.'
  ...
  if (p < 95) return 'Финишная прямая: фронт перевода уже виден.'
