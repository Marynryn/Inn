...
@media (max-width: 560px) {
  ...
  /* Числовые колонки меряются не числом, а своим заголовком: «Попыток» длиннее
     всех, а .num центрирует его без переносов, и лишнее вылезало за ячейку в обе
     стороны. У средних колонок это съедало воздух соседей и оставалось видимым,
     а у крайней уходило за рамку таблицы и обрезалось overflow'ом — на вкладке
     «Свободная игра» последней стоит как раз «Попыток».
  ...
  /* Место и аватарка отдают лишнее имени: из всех колонок только оно умеет
     сжиматься многоточием, и на пять колонок в «Персонаже дня» его надо беречь. */
  .c-place { width: 40px; }
  .c-avatar { width: 32px; }
